...
func NewApp(version string, handler Handler) *App {
	return &App{
		Version: version,
		Handler: handler,
	}
...
func addMigration(c *cli.Context) error {
	...
	migrationFileName, err := AddMigration(description, Format(outputFormat))
	if err == nil {
...
func addTable(c *cli.Context) error {
	...
	updatedMigrationId, err := AddTable(tableName)
	if err != nil {
		return err
	}
...
func deleteTable(c *cli.Context) error {
	...
	updatedMigrationId, err := DeleteTable(tableName)
	if err != nil {
		return err
	}
...
func addColumn(c *cli.Context) error {
	...
	updatedMigrationId, err := AddColumn(tableName, columnName, columnType, isNullable, defaultValue)
	if err != nil {
		return err
	}
...
func deleteColumn(c *cli.Context) error {
	...
	updatedMigrationId, err := DeleteColumn(tableName, columnName)
	if err != nil {
		return err
	}
...
func addPrimaryKey(c *cli.Context) error {
	...
	updatedMigrationId, err := AddPrimaryKey(tableName, columnName)
	if err != nil {
		return err
	}
...
func deletePrimaryKey(c *cli.Context) error {
	...
	updatedMigrationId, err := DeletePrimaryKey(tableName, columnName)
	if err != nil {
		return err
	}
...
func parseColumnsMapping(mappingRaw string) (*[]ColumnsMap, error) {
	mapping := []ColumnsMap{}
	...
	if mappingRaw != "" {
		for _, rawMap := range strings.Split(mappingRaw, ";") {
			...
			mapping = append(mapping, ColumnsMap{
				Column:       column,
...
func addRelation(c *cli.Context) error {
	...
	updatedMigrationId, err := AddRelation(relationName, RelationType(relationType), table, remoteTable, *columnsMapping)
	if err != nil {
		return err
	}
...
func deleteRelation(c *cli.Context) error {
	...
	updatedMigrationId, err := DeleteRelation(table, relationName)
	if err != nil {
		return err
	}
...
func addUniqueConstraint(c *cli.Context) error {
	...
	updatedMigrationId, err := AddUniqueConstraint(constraintName, table, columns)
	if err != nil {
		return err
	}
...
func deleteUniqueConstraint(c *cli.Context) error {
	...
	updatedMigrationId, err := DeleteUniqueConstraint(table, relationName)
	if err != nil {
		return err
	}
...
func migrationSnapshot(c *cli.Context) error {
	snapshot, err := GetCurrentSnapshot()
	if err != nil {
		return err
	}
...
func (a *App) syncMigrations(c *cli.Context) error {
	...
	lastMigrationId, err := Execute(migrations, DBParams{
		User:     userName,
		Password: password,
		Name:     dbName,
		Host:     host,
...
func pingDb(c *cli.Context) error {
	...
	return PingDb(userName, password, dbName, host, port)
}
